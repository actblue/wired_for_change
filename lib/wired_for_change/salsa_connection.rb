...
class SalsaConnection
  ...
  class PostFailure < StandardError
  end
  ...
  def post_succeeded?
    REXML::XPath.first(
      REXML::Document.new(self.raw_post_response),
      "//success"
    ) != nil
  end
  def post(object)
    ...
    if @email && @email != ""
      @auth_resp =
        @conn.post(
          ...
          uri_encode(email: @email, password: @password)
        )
      ...
      @session_cookie =
        @auth_resp["Set-Cookie"] &&
          @auth_resp["Set-Cookie"].split(";").find { |h| h =~ /session/i }

      code = @auth_resp.code
      if code != "200"
        return unless assertive
        raise AuthenticationError,
              "Unexpected response code #{@auth_resp.code} to auth"
      end

      unless @session_cookie && @session_cookie != ""
      ...
      if err =
           REXML::XPath.first(
             REXML::Document.new(@auth_resp.read_body),
             "//error"
           )
        return unless assertive
